...
function createUtilityFunction(propName: string, action: Function) {
  return function(this: undefined | Stamp | Stampit, ...rest: any[]) {
    let tmp: any = {}
    tmp[propName] = action.apply(undefined, concat.apply([{}], rest))
...
allUtilities[PROPERTIES] = allUtilities[PROPS] = createUtilityFunction(PROPERTIES, assign)
...
allUtilities[DEEP_PROPERTIES] = allUtilities[DEEP_PROPS] = createUtilityFunction(
  DEEP_PROPERTIES,
  merge
)

allUtilities[STATIC_PROPERTIES] = allUtilities[STATICS] = createUtilityFunction(
  STATIC_PROPERTIES,
  assign
)

allUtilities[STATIC_DEEP_PROPERTIES] = allUtilities[DEEP_STATICS] = createUtilityFunction(
  STATIC_DEEP_PROPERTIES,
  merge
)

allUtilities[CONFIGURATION] = allUtilities[CONF] = createUtilityFunction(CONFIGURATION, assign)

allUtilities[DEEP_CONFIGURATION] = allUtilities[DEEP_CONG] = createUtilityFunction(
  DEEP_CONFIGURATION,
  merge
)
...
allUtilities[STATIC_PROPERTY_DESCRIPTORS] = createUtilityFunction(
  STATIC_PROPERTY_DESCRIPTORS,
  assign
)
